...
class ProductivePolicyNLJ(object):
    """
    Implements a routing policy that routes tuple to the most productive operators, including NLJ operator in the ordering.
    ...
    def select_operator(self, operators, operators_desc, tup=None, operators_vars=None, operators_not_sym=[]):
        ...
        for operator in operators_selectable:
            ...
            if self.priority_table[operator].input == 0:
                productivity = -1
            else:
                productivity =  self.priority_table[operator].output / input_card
                all_output_zero = False

            if productivity> highest_priority:
                selected_operator = operator
                highest_priority = productivity
