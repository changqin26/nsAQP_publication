...
class EddyNetwork(object):
    ...
    def execute(self, outputqueue):
        ...
        for i in range(1, self.n_eddy+1):
            ...
            p.start()
            self.request_counter.increment()  # <-- Add this line to increment the request counter
...
class IndependentOperator(object):
    ...
    def execute(self, left, right, request_counter, outputqueues):
        ...
        self.p = Process(target=contactLDFServer,
                         args=(self.server, self.query, self.q, self.vars, request_counter,))
...
def contactLDFServer(server, query, queue, vars, request_counter=0):
    #print "server", server
    #print "query", query
