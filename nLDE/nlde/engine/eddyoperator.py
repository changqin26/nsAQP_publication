...
class EddyOperator(object):
    ...
    def __init__(self, id, policy, pool, operators_desc, operators_input_queues,  operators_vars,
                 outputqueue, intermediate_results_counter, independent_sources=0, eofs_operators_desc={}, operators_sym={}, operators=[]):
        self.id = id
        self.policy = policy
        ...
        self.end = False
        self.wait = True
        self.intermediate_results_counter = intermediate_results_counter
    ...
    def execute(self):
        ...
        while not self.end:
            ...
            try:
                ...
                #print("Ops not been executed", operators)
                ...
                if len(operators) == 0:
                    ...
                    if tup.data == "EOF":
                        ...
                        if not self.eof:
                            #print("EOF in eddy", self.id)
                            #print("EOF ready:", tup.ready)
                            #print("EOF done:", tup.done, "\n")
                            self.eof = tup
                            ...
                            for i in range(0, len(self.eddies)):
                                if i != self.id:
                                    ...
                                    #print("Adding eddy {} to tuple {}".format(i, tup.data))

                    # Case: Produce query solution.
                    else:
                        #print("Tuple {} history: {}".format(tup.data, ', '.join(map(str, tup.history))))
                ...
                # Case: Route tuple to pending physical operators.
                    ...
                    #tup.add_to_history(operator)
                    #print("Adding operator {} to tuple {}".format(operator, tup.data))
                ...
                with self.intermediate_results_counter.get_lock():
                    self.intermediate_results_counter.value += 1

            except Empty:

                # Last phase of execution.
                if self.finalize:
                    #print("Finalize eddy", self.id)
                    ...
                    for op in self.operators:
                        #print("Eddy checking", self.id, op.id_operator, op.probing.value)
                        op_active = op_active | op.probing.value

                    # If all the the operators are finished and there are no tuples in the routing buffer,
                    # then finish.
                    #print("Eddy finalize?", self.id, "op active", op_active, "in empy", in_empty)
