...
request_counter = NLDERequestCounter()
...
def get_metadata_ldf(server, query):
    ...
    #print("Request count:", request_counter.get_count())
...
def contact_ldf_server(server, query, queue, request_counter):
    # Extract server information.
    referer = server
    server = server.split("http://")[1]
    (server, path) = server.split("/", 1)
    ...
    while count < total:
        ...
        with request_counter.get_lock():
            #print("Before", request_counter.value)
            request_counter.value += 1
            #print("After", request_counter.value)
        #conn.set_debuglevel(1)
        ...
        if response.status == httplib.OK:
            ...
            params = urllib.urlencode(
                {'subject': subject.value, 'predicate': predicate.value, "object": value, "page": page})
