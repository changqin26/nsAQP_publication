...
from nlde.engine import request_counter
...
#from nlde.engine.contactsource import RequestCounter
...
class DependentOperator(object):
    ...
    def __init__(self, sources, server, query, sources_desc, variables=None, res=0, request_counter=None):
    ...
    def execute(self, variables, instances, outputqueue, p_list=None):
        ...
        aux_queue = Queue()
        self.p = Process(target=contact_ldf_server, args=(self.server, tp, aux_queue, self.request_counter))
        self.p.start()
...
class IndependentOperator(object):
    ...
    def __init__(self, sources, server, query, sources_desc, variables=None, eddies=2, eofs_desc={}, request_counter=0):
        if variables is None:
            variables = []
    ...
    def execute(self, left, right, outputqueues, p_list):
        ...
        # Contact source.
        self.p = Process(target=contact_ldf_server, args=(self.server, self.query, self.q, self.request_counter))
        self.p.start()
...
class TreePlan(object):
    ...
    def __init__(self, operator, variables, join_vars, sources, left, right, height=0, res=0, request_counter=None):
    ...
    def execute(self, operators_input_queues,  eddies_queues, p_list, operators_desc):
        ...
        if self.left:
            ...
            if self.left.__class__.__name__ == "IndependentOperator":
                ...
                s_e = randint(1, len(eddies_queues)-1)
                #print("s_e", s_e)
                #print("eddies_q", eddies_queues)
                p1 = Process(target=self.left.execute,
                             args=(eddies_queues[s_e], None, eddies_queues, p_list,))
        ...
        if self.right and ((self.right.__class__.__name__ == "TreePlan") or (self.right.__class__.__name__ == "IndependentOperator")):
            ...
            if self.right.__class__.__name__ == "IndependentOperator":
                ...
                p2 = Process(target=self.right.execute,
                             args=(None, eddies_queues[s_e], eddies_queues, p_list,))
...
def create_plan(query, eddies, source, request_counter=None):
    ...
    for subquery in query.where.left.triple_patterns:
        ...
        leaf = IndependentOperator(id_operator, source, subquery, sources_desc, subquery.get_variables(), eddies, eofs_desc, request_counter)
                ...
                if isinstance(star_tree, IndependentOperator):
                    ...
                    if star_tree.total_res < (subtree_j.total_res / 100.0):
                        subtree_j = DependentOperator(subtree_j.sources, subtree_j.server, subtree_j.query,
                                                      subtree_j.sources_desc, subtree_j.vars, subtree_j.total_res, request_counter)
                        op = Xnjoin(id_operator, join_variables, eddies)
                        operators.append(op)
                        star_tree = TreePlan(op, all_variables, join_variables, sources,
                                             star_tree, subtree_j, tree_height, 0, request_counter)
                    ...
                    # Place a Symmetric Hash join.
                        ...
                        operators.append(op)
                        star_tree = TreePlan(op, all_variables, join_variables, sources,
                                             star_tree, subtree_j, tree_height, res, request_counter)
                        independent_sources = independent_sources + 1
                    ...
                    if star_tree.total_res == 0 or subtree_j.total_res == 0:
                        pass
                        #print("DEBUG: zero cardinality for a subplan!")
                        #print("STAR TREE pattern:", star_tree.query if hasattr(star_tree, 'query') else "(subtree)",
                              #"TOTAL RES:", star_tree.total_res)
                        #print("SUB TREE pattern:", subtree_j.query if hasattr(subtree_j, 'query') else "(subtree)",
                              #"TOTAL RES:", subtree_j.total_res)
                    #res = (2.0 * star_tree.total_res * subtree_j.total_res) / (star_tree.total_res + subtree_j.total_res)
                    ...
                    if (star_tree.total_res / float(subtree_j.total_res) < 0.30) or (subtree_j.total_res > 100*1000 and star_tree.total_res < 100*1000) or (subtree_j.total_res < 100*5):
                        ...
                        star_tree = TreePlan(op, all_variables,
                                             join_variables, sources, star_tree, subtree_j, tree_height, res, request_counter)
                        independent_sources = independent_sources + 1
                        operators_sym.update({id_operator: True})
                id_operator += 1
                ...
                if isinstance(subtree_j, IndependentOperator):
                    ...
                    if subtree_i.total_res < (subtree_j.total_res/100.0):
                        ...
                        stars.append(TreePlan(op, all_variables,
                                              ...
                                              max(subtree_i.height, subtree_j.height, res, request_counter)))
	...
	#print("Projection variables:", query.projection)

        operators.append(op)
        tree = TreePlan(op,
                        tree.vars, tree.join_vars, tree.sources, tree, None, tree_height+1, tree.total_res, request_counter)
    ...
    if query.distinct:
        ...
        tree = TreePlan(op, tree.vars, tree.join_vars, tree.sources,
                        tree, None, tree_height + 1, tree.total_res, request_counter)
