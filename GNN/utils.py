...
class StatisticsLoader():
    ...
    def __getitem__(self, item):
        try:
            ...
            statistic_dict = {"embedding": [random.uniform(0, 1) for i in range(100)], "occurrence": 0}
...
def get_query_graph_data_new(query_graph, statistics, device, unknown_entity='false', n_atoms: int = None):
    ...
    #print(f"Initial n_entities_random: {n_entities_random}")
    ...
    if shuffled:
        ...
        # Add this print to see the shuffled order:
        #print(f"Shuffled triples: {triple_list}")
    ...
    for triple in query_graph["triples"]:
        ...
        if "?" in triple[1]:
            ...
            try:
                ...
                ea = torch.cat((data["entity", p, "entity"].edge_attr, torch.tensor(v_array)), dim=0)
                ...
                v_array = np.array(v).reshape(1, -1)  # This reshapes v to have one row and n columns

                # Now convert the reshaped array to a tensor
                data["entity", p, "entity"].edge_attr = torch.tensor(v_array)
                #data["entity", p, "entity"].edge_attr = torch.tensor([v])
                #data["entity", p, "entity"].edge_attr = torch.tensor(np.array(v))

                #print("new:", data["entity", p, "entity"].edge_attr)
            ...
            try:
                ...
                # Add the occurrence of the predicate to the embedding
                if USE_OCCURRENCE:
                    feature_vector.append(np.log1p(statistics[triple[1].replace("<", "").replace(">", "")]["occurrence"]))
                else:
        ...
        if not s in node_mapping.keys():
            ...
            n_entities += 1
            if "?" in s:  # If it's a variable
                emb = np.copy(variable_embedding)  # Create a copy of the embedding
                if shuffled:
                    emb[0] = n_entities_random  # Assign a unique ID
                    #print(f"Assigning ID {n_entities_random} to variable {s}")
                else:
                    emb[0] = node_mapping[s]
                n_entities_random += 1
                node_embeddings.append(emb)  # Append the copy to node_embeddings
                #print(f"Embedding for variable {s}: {emb}")
            else:
                # Logic for IRIs remains unchanged
                if unknown_entity == 'false':
                    feature_vector = statistics[s]["embedding"].copy()
                    feature_vector.append(np.log1p(statistics[s]["occurrence"]))
        ...
        if not o in node_mapping.keys():
            ...
            if "?" in o:  # If it's a variable
                ...
                n_entities_random += 1
                node_embeddings.append(emb)  # Append the copy to node_embeddings
                #print(f"Variable: {o}, ID: {n_entities_random - 1}, Embedding: {emb}")
            else:
                # Logic for IRIs remains unchanged
                if unknown_entity == 'false':
                    feature_vector = statistics[o]["embedding"].copy()
                    feature_vector.append(np.log1p(statistics[o]["occurrence"]))
                elif unknown_entity == 'true':
                    feature_vector = unknown_entity_embedding.copy()
                    feature_vector.append(1)
                elif unknown_entity == 'random':
    ...
    # Redirect stdout to a file
        ...
        # Assuming data["entity"].x has been assigned
    data["entity"].x = torch.tensor(np.array(node_embeddings))
