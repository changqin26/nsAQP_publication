...
def prepare_data_list(data_points, statistics, device, num_classes, inductive, is_test=False):
    data_list = []
    n_atoms = 0
    for datapoint in data_points:
        if inductive == 'full' and is_test:
            # Use fixed embeddings for unknown entities in test data
            data, n_atoms = get_query_graph_data_new(
                datapoint, statistics, device, unknown_entity='true', n_atoms=n_atoms
            )
            # Print embeddings for entities in the test data
            #print("Evaluation (inductive='full') entity embeddings:")
            #print(data["entity"].x)  # Prints the embeddings tensor for entities
        else:
            # Default behavior: use precomputed embeddings
            data, n_atoms = get_query_graph_data_new(
...
def plot_probabilities_histogram(tp_probs, fp_probs, dataset_type):
    for cls, tp_probs_cls in tp_probs.items():
        tp_labels = [True] * len(tp_probs_cls)
        fp_probs_cls = fp_probs[cls]
        fp_labels = [False] * len(fp_probs_cls)

        probs_cls = tp_probs_cls + fp_probs_cls
        labels_cls = tp_labels + fp_labels

        data = list(zip(probs_cls, labels_cls))
        tp_data = [prob for prob, label in data if label]
        fp_data = [prob for prob, label in data if not label]

        plt.figure(figsize=(5, 2.5))
        sns.histplot(tp_data, bins=20, kde=True, color='blue', label='True Positives', stat='density', alpha=0.5)
        sns.histplot(fp_data, bins=20, kde=True, color='red', label='False Positives', stat='density', alpha=0.5)
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f'{dataset_type}_class_{cls}_scatter_Final_lubm_inductiveRandom.png')
        plt.show()
...
class PolicyClassification:
    ...
    def train_GNN(
            self, train_data, test_data, thresholds, num_classes=6, epochs=20, train=True,
        ...
        result_path = Path(self.DATASETPATH) / self.dataset_name / "Results" / "GNN" / "Inductive" / starttime
        ...
        if inductive == 'full':
            # For inductive full, training data uses predefined embeddings
            X, n_atoms = prepare_data_list(train_data, self.statistics, self.device, num_classes, inductive,
                                           is_test=False)
            # For test data, use fixed vectors for unknown entities
            X_test, _ = prepare_data_list(test_data, self.statistics, self.device, num_classes, inductive, is_test=True)
        else:
            # For non-inductive modes, use predefined embeddings for both training and test data
            X, n_atoms = prepare_data_list(train_data, self.statistics, self.device, num_classes, inductive,
                                           is_test=False)
            X_test, _ = prepare_data_list(test_data, self.statistics, self.device, num_classes, inductive,
                                          is_test=False)

        preparation_time += time.time() - offline_start_time
        ...
        if train:
            ...
            plot_probabilities_histogram(best_tp_probs, best_fp_probs, "Training")
...
def train_GNCE(dataset: str, query_type: str, eval_folder: str, query_filename: str,
    ...
    results = model.train_GNN(
        ...
        epochs=20, train=train, eval_folder=eval_folder, inductive=inductive,
