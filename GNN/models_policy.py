...
class TripleModelForPolicyClassification(torch.nn.Module):
    def __init__(self, num_classes):
        ...
        self.lin2 = Linear(50, num_classes)  # Output the number of classes

        self.dropout = torch.nn.Dropout(p=0.2)

    def forward(self, x, edge_index, edge_type, edge_attr, batch=None):
        x = F.relu(self.conv1(x, edge_index,
                              edge_attr=edge_attr))  # This line applies the first convolution layer, which processes the input features x along with edge indices and attributes, applying a ReLU activation function afterward.
        x = F.relu(self.conv2(x, edge_index,
                              edge_attr=edge_attr))  # This line further processes the output from the previous convolution layer with another convolution layer and ReLU activation. Up to this point, the operations are focused on feature transformation based on the graph structure.
        x = global_add_pool(x, batch)  # This pooling function aggregates node features into a single graph-level representation. If batch is provided, it will aggregate node features batch-wise, which is typical in scenarios where you are processing multiple graphs at once.
        x = self.lin(x)  # A linear transformation is applied to the pooled graph features.
        # x = self.dropout(x)
        x = F.relu(x)  # ReLU activation function is applied to introduce non-linearity.
        logits = self.lin2(x)   # Now outputs logits for each class
        return logits
